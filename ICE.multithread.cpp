...
void starform(int &new_stars, int time, int cell_no, double gas_mass, vector<vector<vector<vector<double> > > > &iso_mass, double mass_up, double max_mass, vector<vector<int> > &stars, vector<vector<double> > &iso_star, double metallicity, vector<double> &star_mass, vector<double> &age_star, vector<double> &life_star, double star_minmass, double star_maxmass){
    ...
    a = round(dis(gen) * (cell_no-1));
    b = round(dis(gen) * (cell_no-1));
    c = round(dis(gen) * (cell_no-1));
    ...
    gas_mass = accumulate(iso_mass.at(a).at(b).at(c).begin(), iso_mass.at(a).at(b).at(c).end(), 0.0); // gas mass = m(H) + m(He) + m(Z)
    if ( gas_mass >= mass_up ) {
        // Now finding the number of stars in each cell.
        //if ( dis(gen) < (gas_mass/max_mass)) {
        if ( dis(gen) < (pow(gas_mass,1.4)/max_mass)) {
            stars.push_back(newStar);
            stars.at(stars.size()-1).push_back(i);
            stars.at(stars.size()-1).push_back(j);
            stars.at(stars.size()-1).push_back(k);
            
            new_stars++;
            // Check that the current array position doesn't already have a star in it.
            // Calculating stellar lifetimes in units of Myrs.
            star_mass.push_back(salpeter(star_minmass,star_maxmass));
            // Calculate mass percentage of H, He .. Z in gas, multiply percentages by mass of star formed and subtract from iso_mass...
            for (q=0; q<elmax; q++) {
                if (iso_mass.at(a).at(b).at(c).at(q) > (1e-20 + star_mass.at(stars.size()-1) * (iso_mass.at(a).at(b).at(c).at(q) / gas_mass))) {
                    iso_mass.at(i).at(j).at(k).at(q) -= star_mass.at(stars.size()-1) * (iso_mass.at(a).at(b).at(c).at(q) / gas_mass);
                    iso_star.push_back(newColumn);
                    iso_star.at(stars.size()-1).push_back(star_mass.at(stars.size()-1) * (iso_mass.at(a).at(b).at(c).at(q) / gas_mass));
                }else{
                    iso_mass.at(a).at(b).at(c).at(q) = 0.0;
                    iso_star.push_back(newColumn);
                    iso_star.at(stars.size()-1).push_back(star_mass.at(stars.size()-1) * (iso_mass.at(a).at(b).at(c).at(q) / gas_mass));
                }
            ...
            life_star.push_back(round(pow(star_mass.at(stars.size()-1),-2.0)*11.7e3 ));
            age_star.push_back(time);
        }
    }
...
int main() {
    ...
    for ( time=0; time < t_steps; time+=delta_t ){
        ...
        new_stars = 0;
        // POSSIBLY CHANGE REF(...) TO PASS VECTORS TO THREADS DIFFERENTLY
        while (new_stars < star_n) {
            star_threads.push_back(thread(starform, std::ref(new_stars), std::ref(time), std::ref(cell_no), std::ref(gas_mass), std::ref(iso_mass), std::ref(mass_up), std::ref(max_mass), std::ref(stars), std::ref(iso_star), std::ref(metallicity), std::ref(star_mass), std::ref(age_star), std::ref(life_star), std::ref(star_minmass), std::ref(star_maxmass)) );
        }
        for( auto& t : star_threads ) t.join() ;
        ...
        for(i=0;i<stars.size();i++){
            if ( life_star.at(i) <= 0 && star_mass.at(i) != 0 ){ // If a star is about to die.
                if (star_mass.at(i) >= 0.1){
                    SN_Rate[time]++;
                }
                star_dying.push_back(stars.at(i).at(0));
                star_dying.push_back(stars.at(i).at(1));
                star_dying.push_back(stars.at(i).at(2));
                star_ids.push_back(i);

            }
            life_star.at(i)-=delta_t;
        }
        int starpos;
        ...
        while ( !star_dying.empty() ){
            ...
            if ( starpos==0 or (starpos%3)==0 ){
                ...
                for (m = 0; m < elmax; m++){
                    iso_orig.at(m) = iso_star.at(star_ids[starpos/3]).at(m);
                }
                // Extrapolating to stars > 30 M_sun is dangerous, so it is assumed all stars in this mass range end their lives as supernovae.
                if (star_mass.at(star_ids[starpos/3]) <= 30.0){
                    mass_exp(star_mass.at(star_ids[starpos/3]),x,y,z,cell_size,cell_no, iso_mass, iso_orig);
                }
                
                // Erase all info of dead stars from arrays.
                iso_star.erase(iso_star.begin() + star_ids[starpos/3]);
                stars.erase(stars.begin() + star_ids[starpos/3]);
                star_mass.erase(star_mass.begin() + star_ids[starpos/3]);
                life_star.erase(life_star.begin() + star_ids[starpos/3]);
                age_star.erase(age_star.begin() + star_ids[starpos/3]);
...
void print_results( int cell_no, vector<vector<vector<vector<double> > > > &iso_mass, int time, vector<vector<double> > &iso_star, vector<double> &life_star, vector<double> &age_star) {
    ...
    if (print_gas == true){
        ...
        for (i=0; i<cell_no; i++) {
           for (j=0; j<cell_no; j++) {
                   for (m=0; m<elmax; m++) {
                       results << iso_mass.at(i).at(j).at(k).at(m) << " ";
    ...
    if (print_stars == true){
        ...
        for (i=0; i<age_star.size(); i++) {
            if (life_star.at(i) > 0.0 && dis(gen) < 0.1) { // Only printing 10 percent of the stars.
                results << time - age_star.at(i) << " "; // star age
                for (m=0; m<elmax; m++) {
                    results << iso_star.at(i).at(m) << " ";
                }
                results << "\n";
            }
        }
