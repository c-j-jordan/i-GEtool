...
// May need to define this as a tuple if there are too many problems with two returned values...

vector<vector<vector<vector<double> > > > mass_exp (double sn_mass, int x, int y, int z, float cell_size, int cell_no, vector<vector<vector<double> > > &mass, vector<vector<vector<vector<double> > > > &iso_mass);
void print_results (int cell_no, vector<vector<vector<double> > > &mass, int generation);
float yields();
int i,j,k,l;
int Z_num; // Number of metallicities.
float y_Z[5]; // Metallicity.
int y_M[5]; // Number of masses.
...
int main() {
    ...
    double mass_add;
    float nu = 0.006;
    float sfe = 0.25; // 0.25; Argast // 0.02; Original
    int buffer = 5;
    ...
    float SFR;
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    // Allow 0.3125 of all cells to form a star in each timestep, if a random number is less than the SFE... //
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////
    yields();
        ...
        for (i=0; i < cell_no; i++) {
            for (j=0; j < cell_no; j++) {
                for (k=0; k < cell_no; k++) {
                    ...
                    SFR += (cell_size/1000)*cell_no*2.5*pow(10,-4) * pow((mass.at(i).at(j).at(k)),1.4); // /pow(cell_size,3)
                }
            }
        }
        // Divide by average mass of Salpeter IMF (power -2.35) to get total number of stars born per timestep.
        star_n = round(SFR/(0.35*cell_no*cell_no*cell_no)); // 0.35 is the average mass of from a Salpeter IMF.
...
//                            stars[i][j][k] += round(nu*pow(gas_mass,1.5)); // split_vec approach
                        if ( dis(gen) > (star_n/(cell_no*cell_no*cell_no)) && dis(gen) < 0.001  ) {
                            stars[i][j][k]++;
                            new_stars = stars[i][j][k];
                        }
//                            new_stars = round(nu*pow(gas_mass,1.5));
...
//                                    cout << "Stars in cell: " << stars[i][j][k] << endl;
                                ...
                                try {
                                    ...
                                    if ( star_mass[i][j][k][l] == 0 ) {
                                        ...
                                        life_star[i][j][k][l] = round(pow(10,((3.79+0.24*-4) - (3.1+0.35*-4)*log10(star_mass[i][j][k][l])+(0.74+0.11*-4)*pow((log10(star_mass[i][j][k][l])),2)))); // New lifetime law required...
                                        total_stellar_mass[i][j][k] += star_mass[i][j][k][l];
                                        if (life_star[i][j][k][l] >0) {
//                                            cout << "Mass " << star_mass[i][j][k][l] << " and lifetime " << life_star[i][j][k][l] << endl;
                                        }
                                    }
                                }
                                catch (exception& e) {
...
//                                cout << "SN Exploded at gen " << generation << endl;
                                star_mass.at(x).at(y).at(z).at(l) = 0;
        ...
        if ( generation % 1 == 0){
            ...
            print_results(cell_no, mass, generation);
            cout << count << " high mass stars formed.\n";
        }
...
vector<vector<vector<vector<double> > > > mass_exp (double sn_mass, int x, int y, int z, float cell_size, int cell_no, vector<vector<vector<double> > > &mass, vector<vector<vector<vector<double> > > > &iso_mass) {
    ...
    for (i=0; i<Z_num; i++) {
        ...
        if (y_Z[i] > metallicity && y_Z[i-(i/i)] < metallicity ) {
            Z_u = i;
        }
        for (j=0; j<y_M[i]; j++) {
            ...
            if ( stellar_mass[i][j] > sn_mass && stellar_mass[i][j-(j/j)] < sn_mass ) {
                M_u = j;
            }
        }
    }
    ...
    if (int_m == true){
        ...
        for (iso=0;iso<elmax;iso++) {
            iso_exp_Zl[iso] = linint(y_M[M_l],stellar_iso[Z_l][M_l][iso],y_M[M_u],stellar_iso[Z_l][M_u][iso],sn_mass);
            if (int_z == true) {
                iso_exp_Zu[iso] = linint(y_M[M_l],stellar_iso[Z_u][M_l][iso],y_M[M_u],stellar_iso[Z_u][M_u][iso],sn_mass);
...
void print_results( int cell_no, vector<vector<vector<double> > > &mass, int generation) {
    ...
    filename << "mass_t" << generation << ".dat";
