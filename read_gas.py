...
#for tstep in range(0,150):
    cell_no = 100
    cell_size = 25
    #max_r =  # 7 for usual runs ; 0 for one-zone runs
    max_t = 150 #
    max_iso = 280
    ...
    Carbon = np.zeros((cell_no,cell_no),dtype=float)
    ...
    Mg = np.zeros((cell_no,cell_no),dtype=float)
    ...
    Si = np.zeros((cell_no,cell_no),dtype=float)
    ...
    Fe = np.zeros((cell_no,cell_no),dtype=float)
    ...
    SionO_t = np.zeros((cell_no,cell_no,max_t),dtype=float)
    ...
    FeonH = np.zeros((cell_no, cell_no),dtype=float)
    ...
    for i in range(0,cell_no):
        for j in range(0,cell_no):
            ...
            O[i][j] = data[count][5]
            Mg[i][j] = data[count][9]
            Si[i][j] = data[count][11]
            Fe[i][j] = data[count][24]
            metallicity[i][j] = np.sum(data[count][2:])
#                if (mass[i][j][k] <= 0):
#                    mass[i][j][k] += 1e-20
#                    print("Mass of ", mass[i][j][k])
            x[i] = cell_size*i
            y[j] = cell_size*j
            count+=1


    lmetallicity = np.zeros((cell_no,cell_no),dtype=float)
    FeonH_avg = 0.0
...
#            if (H[i][j] < 1e-20):
#                H[i][j] = 1e-20
            FeonH[i][j] =   math.log10(Fe[i][j]/H[i][j]+1e-20) - math.log10( Fe_m_abund/H_m_abund )
            FeonH_avg += FeonH[i][j]
            mass[i][j] = H[i][j] + He[i][j] + metallicity[i][j]
            logmass[i][j] = math.log10(mass[i][j])
            lmetallicity[i][j] = math.log10(metallicity[i][j]+1e-20)
    print("Average [Fe/H] of ", FeonH_avg/(cell_no*cell_no))
...
#                FeonH[i][j] = 0.49
    ...
    py.xlabel("$x$ (pc)")
    py.ylabel("$y$ (pc)")
    py.pcolor(x,y,logmass[:][:])
    py.colorbar(label="log(Mass (M$_\odot$))")
    py.set_cmap('Blues')
    py.xlim(0, 2480)
    py.ylim(0,2480)
    title = 'Mass Distribution at Time ' + str(tstep) + ' Myr'
    py.title(title)
    py.show()
    py.clf()
#
    ...
    py.title("Metallicity")
    py.xlabel("$x$ (pc)")
    py.ylabel("$y$ (pc)")
    py.pcolor(x,y,FeonH[:][:])
    py.colorbar(label="[Fe/H]")
    py.set_cmap('Blues')
    py.xlim(0, 2480)
    py.ylim(0,2480)
    title = 'Metallicity Distribution at Time ' + str(tstep) + ' Myr'
    py.title(title)
    py.show()
    py.clf()

    y,binEdges=np.histogram(mass,bins=75)
