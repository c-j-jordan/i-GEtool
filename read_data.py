...
def read(tstep):
    cell_no = 100
    cell_size = 25
    #max_r = 7 # 7 for usual runs ; 0 for one-zone runs
    #max_t = 498 #
    #max_iso = 78
    #
    #H_m_abund    = 7.057e-1
    ...
    #H = np.zeros((max_r+1,max_t+1),dtype=float)
    #H2 = np.zeros((max_r+1,max_t+1),dtype=float)
    #He = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    #Carbon = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    #Mg = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    #Si = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    #Fe = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    #metallicity = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    #SionFe = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    #FeonH = np.zeros((max_r+1,max_t+1),dtype=float)
    ...
    mass = np.zeros((cell_no,cell_no,cell_no),dtype=float)
    avgmass = np.zeros((cell_no,cell_no),dtype=float)

    logmass = np.zeros((cell_no,cell_no,cell_no),dtype=float)
    avglogmass = np.zeros((cell_no,cell_no),dtype=float)
    ...
    for i in range(0,cell_no):
        for j in range(0,cell_no):
            for k in range(0,cell_no):
                mass[i][j][k] = file.readline()
                if (mass[i][j][k] <= 0):
                    mass[i][j][k] += 1e-20
                logmass[i][j][k] = math.log10(mass[i][j][k])
                x[i] = cell_size*i
                y[j] = cell_size*j
                z[k] = cell_size*k
...
#    print np.min(mass)
    ...
    py.xlabel("$x$ (pc)")
    py.ylabel("$y$ (pc)")
    py.pcolor(x,y,avglogmass)
    py.set_cmap('rainbow')
    py.xlim(0, 2499)
    py.ylim(0,2499)
    title = 'Time ' + str(tstep) + ' Myr'
    py.title(title)
    py.colorbar()
    py.show()
    ...
    #        nzx = np.delete(zx,i)
    #        nzy = np.delete(zy,i)
# Output x y z and mass for each cell, and note the locations of SN events... At an early timestep and a later timestep.
#gen 8->9
#x 23 y 28 z 12
...
#x 72 y 21 z 96
#x 97 y 7 z 9
    f = open('masses_t'+str(tstep)+'.txt', 'wb')
    f.write(bytes("x y z mass\n", 'UTF-8'))
    for i in range(0,cell_no):
        for j in range(0,cell_no):
            for k in range(0,cell_no):
                f.write(bytes(str(z[i]) + " " + str(y[j]) + " " + str(x[k]) + " " + str(mass[i][j][k]) + "\n", 'UTF-8'))
    f.close()