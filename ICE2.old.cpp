...
void mass_exp (double sn_mass, int x, int y, int z, double  cell_size, int cell_no, vector<vector<vector<vector<double> > > > &iso_mass) {
    int cones;
    cones = 8;
    int sn_reach[1000][cones][3];
    int sn_bound[1000][cones][3];
    int cells_interior[cones];
    int boundary_cells[cones];
...
//    }
    ...
    if (sn_mass > 10.0) { // Ensuring there aren't any AGB stars exploding as SN.
        for (seg=0; seg<2.0*PI; seg+=(2.0*PI/(cones/2))) {
        for (phiseg=0; phiseg<PI; phiseg+=(PI/(cones/4))){
            radius = 0;
            mass_swept = 0.0;
            // Use counter instead of seg in cells_interior etc. increment counter by 1 after the while loop.
            while (mass_swept < double(4.25e4/cones)) {
                mass_swept = 0.0;
                cells_interior[loop_counter] = 0;
                boundary_cells[loop_counter] = 0;
                for (i=x-radius; i<=x+radius; i++) {
                    for (j=y-radius; j<=y+radius; j++) {
                        for (k=z-radius; k<=z+radius; k++) {
                            ...
                            // Problem must be with theta or phi values... Decrease radius by 1 when theta in bottom half, increase again afterwards.
                            if (distancex !=0.0 && distancey !=0.0 ){
                                    theta = atan2(distancey,distancex);
                                if (i > x && j > y){
                                    theta = (PI/2.0) - theta;
                                }
                                if (i > x && j < y){
                                ...
                                if (i < x && j < y){
                                    theta = (3.0*PI/2.0) - theta;
                                }
                                if (i < x && j > y){
                                    theta += (3.0*PI/2.0);
                                }
                            }
                            if (distancex == 0.0 && j >= y){
                                theta = 0.0;
                            }
                            if (distancex == 0.0 && j < y){
                                theta = PI;
                            }
                            if (distancey == 0.0 && i >= x){
                                theta = PI/2.0;
                            }
                            if (distancey == 0.0 && i < x){
                                theta = 3.0*PI/2;
                            ...
                            if ( distance != 0.0 ){
                                phi  = acos(double(distancez/distance));
                                if (k < z){
                                    phi = PI - phi;
                                }
                            }
...
//                            if (distance <= distancez){
                            //cout << "T " << theta << " " <<  " P " << phi << endl;
//                            }
                            if ( (distance > 0.0) && (distance < radius*cell_size) && (theta >= seg) && (theta < double(seg+(2.0*PI/(cones/2)))) && (phi >= phiseg) && (phi < double(phiseg+(PI/(cones/4)))) ){
                                mass_swept += iso_mass[l][m][n][0] + iso_mass[l][m][n][1] + iso_mass[l][m][n][8];
                                // Locations of all cells interior to blast.
                                sn_reach[cells_interior[loop_counter]][loop_counter][0] = l;
                                sn_reach[cells_interior[loop_counter]][loop_counter][1] = m;
                                sn_reach[cells_interior[loop_counter]][loop_counter][2] = n;
                                cells_interior[loop_counter]++;
                            }
                            if ( (distance > 0.0) && (distance >= radius*cell_size)  && (distance < (radius+1)*cell_size) && (theta >= seg) && (theta < double(seg+(2.0*PI/(cones/2)))) && (phi >= phiseg) && (phi < double(phiseg+(PI/(cones/4)))) ){
                                // Locations of all cells at the boundary of the blast.
                                sn_bound[boundary_cells[loop_counter]][loop_counter][0] = l;
                                sn_bound[boundary_cells[loop_counter]][loop_counter][1] = m;
                                sn_bound[boundary_cells[loop_counter]][loop_counter][2] = n;
                                boundary_cells[loop_counter]++;
                            }
                        }
                    }
                }
        ...
        cells_interior[0]++;
        sn_reach[cells_interior[0]][0][0] = x;
        sn_reach[cells_interior[0]][0][1] = y;
        sn_reach[cells_interior[0]][0][2] = z;
        ...
        int test;
        for (test=0;test<cones;test++){
        for (i=0; i<cells_interior[test]; i++) {
            for (m=0; m<elmax; m++) {
                iso_int[m] += iso_mass[sn_reach[i][test][0]][sn_reach[i][test][1]][sn_reach[i][test][2]][m];
                mass_interior_orig += iso_int[m];
            }
            cout << sn_reach[i][test][0] << sn_reach[i][test][1] << sn_reach[i][test][2] << endl;

        }
        }
        cout << "Mass interior " << mass_interior_orig << endl;
        double mass_swept;
        mass_swept = 0;
        for (test=0;test<cones;test++){
        for (i=0; i<cells_interior[test]; i++) {
            ...
            for (m=0; m<elmax; m++) {
                iso_swept[m] += iso_mass[sn_reach[i][test][0]][sn_reach[i][test][1]][sn_reach[i][test][2]][m] * (1.0-(mass_interior/mass_interior_orig));
            }
        ...
        for (i=0; i< cells_interior[test]; i++) {
            for (m=0; m<elmax; m++) {
                iso_mass[sn_reach[i][test][0]][sn_reach[i][test][1]][sn_reach[i][test][2]][m] = (iso_int[m]-iso_swept[m])/cells_interior[test];
            }
            
        }
        }
        for (test=0;test<cones;test++){
        for (i=0; i< boundary_cells[test]; i++) {
            for (m=0; m<elmax; m++) {
                iso_mass[sn_bound[i][test][0]][sn_bound[i][test][1]][sn_bound[i][test][2]][m] += (iso_swept[m]/boundary_cells[test]);
            }
        }
        }
