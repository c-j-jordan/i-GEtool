...
//    }
    ...
    // Set cones = 1 for spherical bubbles.
    cones = 10;
    double theta,phi;
    // Array holding phi and theta boundaries for each segment...
    
    // Loop over theta (0,2pi) and phi (0,pi) for all cells... Calculate ijk values according to the theta/phi values, these ijk values will be the ones in the segment.
    int seg;
    if (sn_mass > 10.0 or dis(gen)<1e-3) { // Ensuring there aren't any AGB stars exploding as SN (also now with 1a).

            for (seg=0;seg<cones;seg++){
                radius=0;
                while (mass_swept < (4.25e4/cones)) {
                    mass_swept = 0.0;
                    cells_interior = 0;
                    boundary_cells = 0;
                ...
                for (i=x-radius; i<=x+radius; i++) {
                    for (j=y-radius; j<=y+radius; j++) {
                        for (k=z-radius; k<=z+radius; k++) {
                            ...
                            theta = acos(distancez/sqrt(distancex*distancex + distancey*distancey + distancez*distancez));
                            phi = atan(distancey/distancex);
                            // add second condition to these if statements to determine if the cells are in the correct segment. Use circumference divided by cones. And the mass in the segment isn't greater than 4.24e4/cones.
                            // if theta and phi (spherical coords) are within range of the segment.
                            if ( (distance < radius*cell_size) && (theta >= seg*(2*PI/cones)) && (theta < (seg+1)*(2*PI/cones)) && (phi >= seg*(PI/cones)) && (phi < (seg+1)*(PI/cones) ) ){
                            ...
                            if (distance >= radius*cell_size  && distance < (radius+1)*cell_size && (theta >= seg*(2*PI/cones)) && (theta < (seg+1)*(2*PI/cones)) && (phi >= seg*(PI/cones)) && (phi < (seg+1)*(PI/cones)) ){
