...
int main() {
    ...
    for ( time=0; time < t_steps; time+=delta_t ){
        ...
        mass_add = 1.3447e4 * pow(time_gyr,0.25) * exp(-(time_gyr)/0.5) / (pow(cell_no, 3)); // Integrates to 10^5.
                        ...
                        for (l=0; l<stars[i][j][k]; l++) {
                                ...
                                if ( star_mass[i][j][k][l] == 0 ) {
                                    ...
                                    for (int m=0; m<elmax; m++) {
                                        ...
                                        if (yield_choice == 2){
                                            metallicity = accumulate(iso_mass[i][j][k].begin(), iso_mass[i][j][k].end(),0.0) - (iso_mass[i][j][k][0] + iso_mass[i][j][k][1] + iso_mass[i][j][k][2] + iso_mass[i][j][k][3]);
                                        }
                                    }
                                    if (metallicity==0){
                                        metallicity = 1e-8;
        ...
        while ( star_dying.size() > 0 ){
            ...
            if ( starpos==0 or (starpos%3)==0 ){
                ...
                mass_exp(star_mass.at(x).at(y).at(z).at(star_ids[starpos/3]),x,y,z,cell_size,cell_no, iso_mass);
                star_mass.at(x).at(y).at(z).at(star_ids[starpos/3]) = 0.0;
                for (m = 0; m < elmax; m++) {
                    iso_star.at(x).at(y).at(z).at((star_ids[starpos/3])*elmax + m) = 0.0;
    ...
    switch ( yield_choice ){
        ...
        case 2:
            ...
            */
            ...
            yield_file.open("./Yields/isotope_yield_table_MESA_only_ye.txt");
...
//                    yield_file >> blank >> blank >> m_rem[i][j]; // Use this line if you care about remnant masses.
                    ...
                    yield_file >> blank >> blank >> blank >> blank >> blank;
                    for( k=0;k<elmax; k++){
                        yield_file >> blank >> stellar_iso[i][j][k] >> stellar_iso_orig[i][j][k] >> protno[i][j][k] >> massno[i][j][k];
                    }
                }
            }
            yield_file.close();
