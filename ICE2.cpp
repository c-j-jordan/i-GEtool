...
void mass_exp (double sn_mass, int x, int y, int z, double  cell_size, int cell_no, vector<vector<vector<vector<double> > > > &iso_mass) {
    int sn_reach[8][300][3];
    int sn_bound[8][300][3];
    int cells_interior[8];
    int boundary_cells[8];
    double mass_swept[8][9];
    float radius[8];
...
//                    iso_swept[seg][m] += iso_mass[sn_reach[seg][i][0]][sn_reach[seg][i][1]][sn_reach[seg][i][2]][m] * (1.0-((mass_interior/mass_interior_orig)/8));
        ...
        int cells_int_all = std::accumulate(cells_interior,cells_interior+8,0);
        int cells_bound_all = std::accumulate(boundary_cells,boundary_cells+8,0);
        ...
        for (seg=0; seg<8; seg++) {
            ...
            for (i=0; i<cells_interior[seg]; i++) {
                cell_int_ids[i*8+seg][0] = sn_reach[seg][i][0];
                cell_int_ids[i*8+seg][1] = sn_reach[seg][i][1];
                cell_int_ids[i*8+seg][2] = sn_reach[seg][i][2];
            }
            for (i=0; i<boundary_cells[seg]; i++) {
                cell_bound_ids[i*8+seg][0] = sn_bound[seg][i][0];
                cell_bound_ids[i*8+seg][1] = sn_bound[seg][i][1];
                cell_bound_ids[i*8+seg][2] = sn_bound[seg][i][2];
            }
        }
